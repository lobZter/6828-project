...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	// Try mapping page from sender to receiver (if receiver wants it, and sender wants to send it)
	...
	if ((uint32_t) rcv->env_ipc_dstva < UTOP && (uint32_t) srcva < UTOP) {
		...
		if (page_insert(rcv->env_pgdir, pp, rcv->env_ipc_dstva, perm) < 0)
			return -E_NO_MEM;
	}
...
sys_copy_mem(envid_t dst_id, void* dst, void* src)
{
	...
	if (sys_page_map(dst_id, mapva, curenv->env_id, (void *) UTEMP, 
			 PTE_U | PTE_P| PTE_W) < 0)
		return -E_INVAL;
